...
const SMOKE_MARKER_PATH = 'smoke_runs/marco5e-refund-sandbox-v2';
...
async function readJson(response, label) {
  ...
  if (!response.ok) {
    const message = body?.error?.message || body?.error?.status || body?.message ||
...
async function main() {
  ...
  const asaasApiKey = readSecret(ASAAS_API_SECRET);
  if (!asaasApiKey.startsWith(EXPECTED_API_KEY_PREFIX)) fail('ASAAS_API_KEY nao possui prefixo Sandbox esperado.');

  function describeFunction(name) {
    const description = JSON.parse(command('gcloud', [
      'functions', 'describe', name,
      '--gen2', `--region=${REGION}`, `--project=${EXPECTED_PROJECT}`, '--format=json'
    ]));
  ...
  async function asaas(pathname, options = {}) {
    return readJson(await fetch(`${ASAAS_BASE_URL}${pathname}`, {
      ...options,
      headers: {
        accept: 'application/json', access_token: asaasApiKey,
  ...
  if (await getDoc(SMOKE_MARKER_PATH)) {
    console.log('MARCO5E_STAGING_REFUND_V2_SMOKE=BLOCKED_ALREADY_STARTED');
  ...
  const now = new Date();
  const buyerEmail = `bjjexams-marco5e-refund-v2-buyer-${runId}@example.test`;
  const adminEmail = `bjjexams-marco5e-refund-v2-admin-${runId}@example.test`;
  ...
  const targetCourseId = `smoke-marco5e-refund-v2-${runId}`;
  const fundingCourseId = `smoke-marco5e-refund-v2-funding-${runId}`;
  const targetKey = `smoke-marco5e-refund-v2-${runId}`;
  const fundingKey = `smoke-marco5e-refund-v2-funding-${runId}`;
  const markerBase = {
    smokeType: 'marco5e-asaas-sandbox-full-refund-v2', runId, projectId: EXPECTED_PROJECT,
  ...
  try {
    ...
    adminApp = initializeApp({ credential: applicationDefault(), projectId: EXPECTED_PROJECT }, `marco5e-refund-v2-${process.pid}-${Date.now()}`);
    ...
    await putDoc(`usuarios/${buyerUid}`, {
      nome: 'BJJ Exams Marco 5.5 Refund V2 Buyer', email: buyerEmail, cpf: generateCpf(),
    ...
    await putDoc(`courses/${targetCourseId}`, { ...courseBase, title: 'Smoke Marco 5.5 - Refund V2 Sandbox', priceCents: TARGET_PRICE_CENTS });
    await putDoc(`courses/${fundingCourseId}`, { ...courseBase, title: 'Smoke Marco 5.5 - Funding V2 Sandbox', priceCents: FUNDING_PRICE_CENTS });
    ...
    async function checkout(courseId, idempotencyKey, priceCents) {
      ...
      return { ...result, orderId, transactionId, enrollmentId };
    }
    ...
    await putDoc(SMOKE_MARKER_PATH, {
      ...markerBase, status: 'target_checkout_pending', buyerUserId: buyerUid, adminUserId: adminUid,
      orderId: target.orderId, transactionId: target.transactionId, updatedAt: new Date()
    });
    ...
    const refundResult = await callFunction(refundUri, adminIdToken, {
      ...
      reason: 'Smoke Marco 5.5 V2 - estorno integral em staging'
    }, REFUND_FUNCTION);
    ...
    const outcome = await waitUntil('resultado terminal do refund', async () => {
      const [order, tx, enrollment, request, events] = await Promise.all([
        getDoc(`orders/${target.orderId}`),
        getDoc(`payment_transactions/${target.transactionId}`),
        getDoc(`enrollments/${target.enrollmentId}`),
        getDoc(`financial_reversal_requests/${refundResult.requestId}`),
        listCollection('payment_webhook_events')
      ]);
      const targetEvents = events.filter(item => item.data.providerPaymentId === target.paymentId && item.data.status === 'processed');
      const refundedEvent = targetEvents.find(item => item.data.eventType === 'PAYMENT_REFUNDED') || null;
      const deniedEvent = targetEvents.find(item => item.data.eventType === 'PAYMENT_REFUND_DENIED') || null;
      const partialEvent = targetEvents.find(item => item.data.eventType === 'PAYMENT_PARTIALLY_REFUNDED') || null;

      if (
        request?.data.status === 'completed' &&
        request.data.providerLifecycleStatus === 'refunded' &&
        order?.data.status === 'refunded' &&
        tx?.data.status === 'refunded' &&
        enrollment?.data.status === 'refunded' &&
        refundedEvent
      ) {
        return { ok: true, value: { kind: 'REFUNDED', order, tx, enrollment, request, event: refundedEvent } };
      }

      if (
        request?.data.status === 'needs_reconciliation' &&
        request.data.providerLifecycleStatus === 'denied' &&
        request.data.errorCode === 'REVERSAL_PROVIDER_REFUND_DENIED' &&
        order?.data.status === 'paid' &&
        tx?.data.status === 'paid' &&
        ['active', 'completed'].includes(enrollment?.data.status) &&
        deniedEvent
      ) {
        return { ok: true, value: { kind: 'REFUND_DENIED', order, tx, enrollment, request, event: deniedEvent } };
      }

      if (
        request?.data.status === 'needs_reconciliation' &&
        request.data.providerLifecycleStatus === 'partial_refund' &&
        request.data.errorCode === 'REVERSAL_PROVIDER_PARTIAL_REFUND' &&
        order?.data.status === 'paid' &&
        tx?.data.status === 'paid' &&
        ['active', 'completed'].includes(enrollment?.data.status) &&
        partialEvent
      ) {
        return { ok: true, value: { kind: 'PARTIAL_REFUND_REVIEW', order, tx, enrollment, request, event: partialEvent } };
      }

      return {
        ok: false,
        debug: {
          order: order?.data.status || null,
          tx: tx?.data.status || null,
          enrollment: enrollment?.data.status || null,
          request: request?.data.status || null,
          providerLifecycleStatus: request?.data.providerLifecycleStatus || null,
          events: targetEvents.map(item => item.data.eventType).sort()
        }
      };
    });

    const archivedAt = new Date();
    await putDoc(`courses/${targetCourseId}`, { ...courseBase, title: 'Smoke Marco 5.5 - Refund V2 Sandbox', priceCents: TARGET_PRICE_CENTS, status: 'archived', updatedAt: archivedAt });
    await putDoc(`courses/${fundingCourseId}`, { ...courseBase, title: 'Smoke Marco 5.5 - Funding V2 Sandbox', priceCents: FUNDING_PRICE_CENTS, status: 'archived', updatedAt: archivedAt });

    await putDoc(SMOKE_MARKER_PATH, {
      ...markerBase, status: 'completed', outcome: outcome.kind,
      buyerUserId: buyerUid, adminUserId: adminUid,
      orderId: target.orderId, transactionId: target.transactionId,
      reversalRequestId: refundResult.requestId, fundingCreated,
      fundingOrderId: funding?.orderId || null,
      balanceBeforeRefundCents: balanceBeforeRefund,
      webhookEventId: outcome.event.id,
      courseArchived: true, updatedAt: archivedAt
    });

    console.log('MARCO5E_STAGING_REFUND_V2_SMOKE=OK');
    ...
    console.log(`REFUND_OUTCOME=${outcome.kind}`);
    console.log(`REVERSAL_REQUEST_STATUS=${outcome.request.data.status}`);
    console.log(`REVERSAL_PROVIDER_LIFECYCLE_STATUS=${outcome.request.data.providerLifecycleStatus}`);
    console.log(`WEBHOOK_EVENT_TYPE=${outcome.event.data.eventType}`);
    console.log(`WEBHOOK_EVENT_STATUS=${outcome.event.data.status}`);
    console.log(`ORDER_STATUS=${outcome.order.data.status}`);
    console.log(`TRANSACTION_STATUS=${outcome.tx.data.status}`);
    console.log(`ENROLLMENT_STATUS=${outcome.enrollment.data.status}`);
...
main().catch(error => {
  console.error(`MARCO5E_STAGING_REFUND_V2_SMOKE=FAILED | ${error.message}`);
  process.exitCode = 1;
});
