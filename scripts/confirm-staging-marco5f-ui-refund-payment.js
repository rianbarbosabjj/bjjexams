...
const COURSE_TITLE = 'Smoke Marco 5.5 - Refund Sandbox';
const PRICE_CENTS = 500;
const ALLOW_ENV = 'BJJ_EXAMS_ALLOW_ASAAS_SANDBOX_UI_REFUND_PAYMENT_CONFIRM';
...
async function main() {
  ...
  try {
    ...
    for (const entry of buyerOrders) {
      ...
      if (!courseId || entry.data?.status !== 'pending_payment') continue;
      const course = await getDoc(`courses/${courseId}`);
      if (
        course?.data?.title === COURSE_TITLE &&
        Number(entry.data?.amountCents) === PRICE_CENTS
      ) {
    ...
    matching.sort((a, b) =>
      timeValue(b.order.data?.createdAt || b.order.data?.updatedAt) -
      timeValue(a.order.data?.createdAt || a.order.data?.updatedAt)
    );

    if (matching.length !== 1) {
      fail(`Esperado exatamente 1 pedido Refund pending do aluno de smoke; encontrado ${matching.length}.`);
    }
    ...
    if (enrollmentBefore) {
      fail('Enrollment ja existe antes da confirmacao do pagamento Refund.');
    }
    ...
    if (
      ...
      fail(
        'Timeout aguardando fulfillment Refund. ' +
    ...
    const courseOrders = (await listCollection('orders'))
      .filter(entry =>
        entry.data?.buyerUserId === buyer.id &&
        entry.data?.productType === 'course' &&
    ...
    const targetTransactions = (await listCollection('payment_transactions'))
      .filter(entry => entry.data?.orderId === orderEntry.id);

    const cancelledPriorOrders = courseOrders.filter(entry => entry.data?.status === 'cancelled');
    const paidOrders = courseOrders.filter(entry => entry.data?.status === 'paid');

    console.log('MARCO5F_UI_REFUND_PAYMENT_CONFIRM=OK');
    ...
    console.log(`COURSE_ORDER_COUNT=${courseOrders.length}`);
    console.log(`PRIOR_CANCELLED_ORDER_COUNT=${cancelledPriorOrders.length}`);
    console.log(`PAID_ORDER_COUNT=${paidOrders.length}`);
    console.log(`TARGET_ORDER_STATUS=${finalState.order.status}`);
    console.log(`TARGET_TRANSACTION_COUNT=${targetTransactions.length}`);
    console.log(`TARGET_TRANSACTION_STATUS=${finalState.transaction.status}`);
    ...
    console.log(`TARGET_ORDER_DUPLICATED=${bool(paidOrders.length !== 1)}`);
    console.log(`TARGET_TRANSACTION_DUPLICATED=${bool(targetTransactions.length !== 1)}`);
...
main().catch(error => {
  console.error(`MARCO5F_UI_REFUND_PAYMENT_CONFIRM=FAILED | ${error.message}`);
  process.exitCode = 1;
});
