...
const app = initializeApp(
  { projectId },
  `financial-checkout-persistence-${process.pid}-${Date.now()}`
);
...
function paidCourse(ownerId, overrides = {}) {
  return {
...
async function seedCheckout({ buyer, owner, courseId, walletId = id('wallet') }) {
  await Promise.all([
    db.doc(`usuarios/${buyer}`).set({ nome: 'BUYER', email: `${buyer}@example.test` }),
  ...
  try {
    await test('customer lease converge e binding ready fica idempotente', async () => {
      ...
      const completed = await api.completeProviderCustomerBinding({
        ...
        providerCustomerId: id('cus')
      });
      const retry = await api.acquireProviderCustomerLease({ userId: buyer });

      assert.equal(completed.completed, true);
      assert.equal(retry.ready, true);
      assert.equal(retry.providerCustomerId, completed.providerCustomerId);
      const audits = await auditsFor(buyer, 'financial.provider_customer.bound');
      assert.equal(audits.length, 1);
    });

    await test('customer lease expirada pode ser retomada sem criar outro documento', async () => {
      const buyer = id('buyer_customer_expired');
    ...
    await test('prepare cria transacao canonica split snapshot e lease antes do provider', async () => {
      ...
      const api = service();
      const result = await api.prepareCourseCheckout({
        buyerUserId: buyer,
        courseId,
      ...
      assert.equal(result.transaction.provider, 'asaas');
      ...
      assert.equal(orderSnap.data().currentTransactionId, result.transactionId);
      const enrollmentSnap = await db.collection('enrollments')
        .where('userId', '==', buyer).get();
      assert.equal(enrollmentSnap.empty, true);
    });

    await test('duas preparacoes da mesma intencao convergem para uma transacao e uma lease ativa', async () => {
      ...
      const audits = await auditsFor(buyer, 'financial.transaction.created');
      assert.equal(audits.length, 1);
    });

    await test('checkout revalida existencia da identidade do recebedor', async () => {
      ...
      await assert.rejects(
        service().prepareCourseCheckout({
          buyerUserId: buyer,
          courseId,
          idempotencyKey: id('intent_deleted_owner')
        }),
        error => error instanceof FinancialCheckoutPersistenceError &&
          error.code === 'RECIPIENT_IDENTITY_REQUIRED'
      );
      const txId = paymentTransactionId({
        provider: 'asaas',
        orderId: require('../src/finance/financial-order-service').financialOrderId({
          buyerUserId: buyer,
          courseId,
          idempotencyKey: id('intent_deleted_owner')
        })
    ...
    await test('release da checkout lease permite retomada segura da mesma transacao', async () => {
    ...
    await test('binding customer divergente falha fechado sem sobrescrever provider id', async () => {
      ...
      await api.completeProviderCustomerBinding({
        ...
        providerCustomerId: id('cus_original')
      });
